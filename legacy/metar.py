...
def get_custom_date_selector(input_day: str) -> str:
    ...
    if int(input_day) == current_day:
        ...
        custom_date_selector = f"{current_month}/{input_day}/{current_year}"
...
@retry(
    ...
    wait=wait_fixed(3),  # Wait 2 seconds between retries
    retry=retry_if_exception_type(PlaywrightTimeoutError)  # Only retry on timeout
)
def select_station_and_observer(page):
    ...
    # Add timeout to the observer selection
    page.get_by_role("option", name="Zulkifli Ramadhan").click(timeout=10000)
...
def fill_form(page, user_input):
    try:
        ...
        select_station_and_observer(page)
        ...
        # time.sleep(1800)
        # page.get_by_role("button", name="Send to Gts Switching").click()
        # page.get_by_role("button", name="OK").click()
        logging.info("Form Send completed.")

        # step 12: Submit
        logging.info("Clicking Submit button...")
        page.get_by_role("button", name="Submit").click()
        logging.info("Form Submit completed.")
...
def reload_browser_page(manager, browser_page):
    ...
    try:
        ...
        browser_page.wait_for_timeout(1000)  # Wait for 3 seconds
